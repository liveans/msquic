...
.PARAMETER MsQuicRepoPath
    Path to the msquic repository. Auto-detected if not specified.

.PARAMETER ArtifactsPath
    Path to build artifacts. Auto-detected if not specified.

.PARAMETER TestArtifactsPath
    Path to test artifacts containing msquictest binary (e.g., artifacts/bin/linux/x64_Release_openssl3).
    Auto-detected from MsQuicRepoPath if not specified.
    Falls back to cached artifacts from previous -BuildTestArtifacts runs in %TEMP%\msquic-test-artifacts.

.PARAMETER BuildTestArtifacts
    Build msquic test artifacts (msquictest) in a Docker container before validation.
    Uses Ubuntu 24.04 container to build for Linux.

.PARAMETER Config
    Build configuration when using -BuildTestArtifacts. Default is Release.

.PARAMETER Tls
    TLS provider when using -BuildTestArtifacts. Default is quictls.
    Valid values: openssl, quictls
...
.PARAMETER SkipDotNetTest
    Skip the .NET QUIC validation test. By default, if msquic repo is available,
    the script will build and run QuicHello .NET test to verify QUIC functionality.
...
.EXAMPLE
    # With msquictest validation (matches validate-msquic-dotnet.sh behavior)
    .\validate-msquic-docker.ps1 -Distro Fedora_40 -TestArtifactsPath C:\msquic\artifacts\bin\linux\x64_Release_openssl3

.EXAMPLE
    # Build test artifacts in Docker and then validate
    .\validate-msquic-docker.ps1 -BuildTestArtifacts -Distro Ubuntu_24_04
...
# Script is in msquic/scripts/, so parent is the repo root
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = Split-Path -Parent $ScriptDir

# Auto-detect MsQuicRepoPath (this script is inside the msquic repo)
if (-not $MsQuicRepoPath) {
    $MsQuicRepoPath = $RepoRoot
}

# Auto-detect ArtifactsPath
if (-not $ArtifactsPath) {
    $artifactsCandidate = Join-Path $MsQuicRepoPath 'artifacts'
    if (Test-Path $artifactsCandidate) {
        $ArtifactsPath = $artifactsCandidate
    }
}
...
if (-not $PackagesPath) {
    $possiblePackagePaths = @(
        (Join-Path $MsQuicRepoPath 'packages'),
        (Join-Path $env:USERPROFILE 'Desktop\msquic-packages')
    )
    if ($ArtifactsPath) {
        $possiblePackagePaths += (Join-Path $ArtifactsPath 'packages')
    }
...
# Auto-detect TestArtifactsPath from msquic repo
if (-not $TestArtifactsPath -and $MsQuicRepoPath -and (Test-Path $MsQuicRepoPath)) {
    # Look for Linux test artifacts: artifacts/bin/linux/{arch}_{config}_{tls}
    $linuxBinPath = Join-Path $MsQuicRepoPath 'artifacts' 'bin' 'linux'
    if (Test-Path $linuxBinPath) {
        # Try to find matching configuration
        $archName = if ($Arch -eq 'x64') { 'x64' } else { 'arm64' }
        $possibleConfigs = @(
            "${archName}_${Config}_${Tls}",
            "${archName}_Release_openssl3",
            "${archName}_Release_openssl",
            "${archName}_Debug_openssl3",
            "${archName}_Debug_openssl"
        )
        foreach ($configDir in $possibleConfigs) {
            $testPath = Join-Path $linuxBinPath $configDir
            $msquicTestPath = Join-Path $testPath 'msquictest'
            if (Test-Path $msquicTestPath) {
                $TestArtifactsPath = $testPath
                break
            }
        }
    }
...
# Function to build msquic test artifacts in Docker
function Build-MsQuicTestArtifacts {
    param(
        [string]$OutputPath,
        [string]$Arch,
        [string]$Config,
    ...
    Write-Header "Building MsQuic Test Artifacts in Docker"
    
    # Create output directory for artifacts
    if (-not $OutputPath) {
        $OutputPath = Join-Path $env:TEMP 'msquic-test-artifacts'
    }
    ...
    $archName = if ($Arch -eq 'x64') { 'x64' } else { 'arm64' }
    $artifactSubdir = "${archName}_${Config}_${Tls}"
    $finalOutputPath = Join-Path $OutputPath $artifactSubdir
    
    # Check if we already have built artifacts
    if (Test-Path (Join-Path $finalOutputPath 'msquictest')) {
        Write-Step "Using cached test artifacts from: $finalOutputPath"
        return $finalOutputPath
    }
    
    Write-Step "Configuration: $Arch $Config $Tls"
    Write-Step "Output: $finalOutputPath"
    
    # Use Ubuntu 24.04 for building
    $buildImage = 'mcr.microsoft.com/dotnet/sdk:9.0-noble'
    
    Write-Step "Pulling build image: $buildImage"
    docker pull $buildImage 2>&1 | ForEach-Object { Write-Info $_ }
    
    if ($LASTEXITCODE -ne 0) {
        Write-ErrorMsg "Failed to pull build image"
        return $null
    }
    
    # Create a temporary directory for output on host
    $tempOutputMount = Join-Path $env:TEMP "msquic-build-output-$(Get-Random)"
    New-Item -ItemType Directory -Path $tempOutputMount -Force | Out-Null
    $outputMount = $tempOutputMount -replace '\\', '/'
    
    # Build script to run inside container - clone fresh from GitHub
    $buildScriptLines = @(
        'set -e',
        'echo "=== Creating sudo wrapper (running as root) ==="',
        'echo ''#!/bin/bash'' > /usr/local/bin/sudo',
        'echo ''exec "$@"'' >> /usr/local/bin/sudo',
        'chmod +x /usr/local/bin/sudo',
        'echo "=== Installing build dependencies ==="',
        'apt-get update -qq',
        'apt-get install -y --no-install-recommends cmake build-essential liblttng-ust-dev lttng-tools libssl-dev git libnuma-dev',
        'echo "=== Cloning msquic from GitHub ==="',
        'cd /tmp',
        'git clone --recursive --depth 1 https://github.com/microsoft/msquic.git',
        'cd /tmp/msquic',
        'echo "=== Verifying submodules ==="',
        'ls -la submodules/googletest/ | head -5',
        'ls -la submodules/clog/ | head -5',
        'echo "=== Running prepare-machine.ps1 ==="',
        'pwsh -Command "./scripts/prepare-machine.ps1 -ForBuild"',
        "echo ""=== Building msquic ($archName $Config $Tls) ===""",
        "pwsh -Command ""./scripts/build.ps1 -Arch $archName -Config $Config -Tls $Tls -DisablePerf""",
        'echo "=== Build complete ==="',
        "ls -la /tmp/msquic/artifacts/bin/linux/${artifactSubdir}/",
        'echo "=== Copying artifacts to output ==="',
        "cp -r /tmp/msquic/artifacts/bin/linux/${artifactSubdir}/* /output/",
        'ls -la /output/',
        'echo "=== Done ==="'
    )
    $buildScript = $buildScriptLines -join "`n"
    
    Write-Step "Building in Docker container..."
    Write-Info "This may take several minutes (cloning and building from scratch)..."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Remove-Item -Path $tempOutputMount -Recurse -Force -ErrorAction SilentlyContinue
        return $null
    }
    
    # Move artifacts to final location
    if (Test-Path (Join-Path $tempOutputMount 'msquictest')) {
        if (-not (Test-Path $finalOutputPath)) {
            New-Item -ItemType Directory -Path $finalOutputPath -Force | Out-Null
        }
        Copy-Item -Path "$tempOutputMount\*" -Destination $finalOutputPath -Recurse -Force
        Remove-Item -Path $tempOutputMount -Recurse -Force -ErrorAction SilentlyContinue
        Write-Success "Build successful: $finalOutputPath"
        return $finalOutputPath
    } else {
        Write-ErrorMsg "msquictest not found in build output"
        Write-Info "Contents of output directory:"
        Get-ChildItem -Path $tempOutputMount -Recurse | ForEach-Object { Write-Info $_.FullName }
        Remove-Item -Path $tempOutputMount -Recurse -Force -ErrorAction SilentlyContinue
        return $null
    }
}
...
Write-Info "Build Config: $Config"
Write-Info "TLS Provider: $Tls"
Write-Info "Packages Path: $(if ($PackagesPath) { $PackagesPath } else { '(not found)' })"
Write-Info "MsQuic Repo: $(if ($MsQuicRepoPath -and (Test-Path $MsQuicRepoPath)) { $MsQuicRepoPath } else { '(not found)' })"
Write-Info "Artifacts Path: $(if ($ArtifactsPath -and (Test-Path $ArtifactsPath)) { $ArtifactsPath } else { '(not found)' })"
Write-Info "Test Artifacts: $(if ($TestArtifactsPath -and (Test-Path $TestArtifactsPath)) { $TestArtifactsPath } else { '(not provided - will auto-detect or build)' })"
Write-Info "Build Test Artifacts: $BuildTestArtifacts"
...
# Build test artifacts if requested, otherwise check cached location
$defaultArtifactsCache = Join-Path $env:TEMP 'msquic-test-artifacts'
if ($BuildTestArtifacts) {
    # Output artifacts to a standard location
    $builtArtifacts = Build-MsQuicTestArtifacts -OutputPath $defaultArtifactsCache -Arch $Arch -Config $Config -Tls $Tls
    if ($builtArtifacts) {
        $TestArtifactsPath = $builtArtifacts
    } else {
        Write-ErrorMsg "Failed to build test artifacts"
        exit 1
    }
} elseif (-not $TestArtifactsPath) {
    # Check if cached test artifacts exist from a previous build
    $archName = if ($Arch -eq 'x64') { 'x64' } elseif ($Arch -eq 'arm64') { 'arm64' } else { 'x64' }
    $cachedPath = Join-Path $defaultArtifactsCache "${archName}_${Config}_${Tls}"
    $cachedMsquictest = Join-Path $cachedPath 'msquictest'
    if (Test-Path $cachedMsquictest) {
        Write-Step "Found cached test artifacts: $cachedPath"
        $TestArtifactsPath = $cachedPath
    }
}
...
if (-not $PackagesPath) {
    ...
    Write-Info "Please specify -PackagesPath parameter or place packages in one of:"
    Write-Info "  - $WorkspaceRoot\packages"
...
if ($runDotNetTests) {
    ...
    if (-not $MsQuicRepoPath -or -not (Test-Path $MsQuicRepoPath)) {
        ...
        Write-Info "To run .NET tests, ensure msquic repo is available at: $MsQuicRepoPath"
            ...
            foreach ($netVersion in $dotnetVersionsToBuild) {
                ...
                if (Test-Path $projectPath) {
                    ...
                    try {
                        ...
                        $buildOutput = & dotnet publish $projectPath -c $Config -o $DotNetTestPath -f "net$netVersion" --self-contained false 2>&1
